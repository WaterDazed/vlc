                        ...
                        const vlc_ancillary_array *src_array)
{
    ...
    bool success = vlc_vector_push_hole(dst_array, src_array->size);
    if (!success)
        return VLC_ENOMEM;

    for (size_t i = 0; i < src_array->size; ++i)
    {
        struct vlc_ancillary *src_ancillary = src_array->data[i];
        dst_array->data[i] = vlc_ancillary_Hold(src_ancillary);
        assert(dst_array->data[i] != NULL);
    }
...
int
vlc_ancillary_array_Move(vlc_ancillary_array *dst_array,
                         vlc_ancillary_array *src_array)
{
    ...
    {
        struct vlc_ancillary *ancillary = src_array->data[i];
        ret = vlc_ancillary_array_Insert(dst_array, ancillary);
    }
