...
vlc_module_begin ()
    vlc_set_description("Video decoder using Android MediaCodec via NDK")
    vlc_set_subcategory(SUBCAT_INPUT_VCODEC)
    vlc_set_section(N_("Decoding"), NULL)
    vlc_set_capability("video decoder", 800)
    vlc_add_bool("mediacodec", true, MEDIACODEC_ENABLE_TEXT, NULL)
    vlc_add_bool(CFG_PREFIX "dr", true,
             DIRECTRENDERING_TEXT, DIRECTRENDERING_LONGTEXT)
    vlc_add_bool(CFG_PREFIX "audio", false,
             MEDIACODEC_AUDIO_TEXT, MEDIACODEC_AUDIO_LONGTEXT)
    vlc_add_bool(CFG_PREFIX "tunneled-playback", false,
             MEDIACODEC_TUNNELEDPLAYBACK_TEXT, NULL)
    vlc_set_callbacks(OpenDecoderNdk, CloseDecoder)
    vlc_add_shortcut("mediacodec_ndk")
    vlc_add_submodule ()
        vlc_set_capability("audio decoder", 0)
        vlc_set_callbacks(OpenDecoderNdk, CloseDecoder)
        vlc_add_shortcut("mediacodec_ndk")
    vlc_add_submodule ()
        vlc_set_description("Video decoder using Android MediaCodec via JNI")
        vlc_set_capability("video decoder", 0)
        vlc_set_callbacks(OpenDecoderJni, CloseDecoder)
        vlc_add_shortcut("mediacodec_jni")
    vlc_add_submodule ()
        vlc_set_capability("audio decoder", 0)
        vlc_set_callbacks(OpenDecoderJni, CloseDecoder)
        vlc_add_shortcut("mediacodec_jni")
