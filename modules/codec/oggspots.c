...
vlc_module_begin ()
    ...
    vlc_set_shortname("OggSpots")
    vlc_set_description(N_("OggSpots video decoder"))
    vlc_set_capability("video decoder", 10)
    vlc_set_callbacks(OpenDecoder, CloseDecoder)
    vlc_add_shortcut("oggspots")

    vlc_add_submodule ()
    vlc_set_description(N_("OggSpots video packetizer"))
