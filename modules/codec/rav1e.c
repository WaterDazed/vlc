...
vlc_module_begin()
    vlc_set_shortname("rav1e")
    vlc_set_description(N_("rav1e video encoder"))
    vlc_set_capability("video encoder", 105)
    vlc_set_callback(OpenEncoder)
    vlc_set_subcategory(SUBCAT_INPUT_VCODEC)
    /* Note: Skip label translation for these - too technical */
    vlc_add_integer(SOUT_CFG_PREFIX "profile", 0, "Profile", NULL)
        vlc_change_integer_range(0, 3)
    vlc_add_integer(SOUT_CFG_PREFIX "bitdepth", 8, "Bit Depth", NULL)
        vlc_change_integer_list(bitdepth_values_list, bitdepth_values_name_list)
