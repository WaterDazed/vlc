...
vlc_module_begin ()
    ...
    vlc_set_shortname( N_("TTML decoder"))
    vlc_set_description( N_("TTML subtitles decoder") )
    vlc_set_callback( tt_OpenDecoder )
    vlc_set_subcategory( SUBCAT_INPUT_SCODEC )
    vlc_add_integer( "ttml-align", 0, ALIGN_TEXT, ALIGN_LONGTEXT )

    vlc_add_submodule()
        vlc_set_shortname( N_("TTML") )
        vlc_set_description( N_("TTML demuxer") )
        vlc_set_capability( "demux", 11 )
        vlc_set_subcategory( SUBCAT_INPUT_DEMUX )
        vlc_set_callbacks( tt_OpenDemux, tt_CloseDemux )
        vlc_add_shortcut( "ttml" )
#ifdef ENABLE_SOUT
    vlc_add_submodule()
        vlc_set_shortname( N_("TTML") )
        vlc_set_description( N_("TTML encoder") )
        vlc_set_capability( "spu encoder", 101 )
        vlc_set_subcategory( SUBCAT_INPUT_SCODEC )
        vlc_set_callbacks( tt_OpenEncoder, NULL )
#endif
vlc_module_end ()
