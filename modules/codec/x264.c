...
vlc_module_begin ()
#ifdef PLUGIN_X264_10B
    vlc_set_description( N_("H.264/MPEG-4 Part 10/AVC encoder (x264 10-bit)"))
    vlc_set_capability( "video encoder", 0 )
#endif
#ifdef PLUGIN_X262
    vlc_set_description( N_("H.262/MPEG-2 encoder (x262)"))
    vlc_set_capability( "video encoder", 0 )
#endif
#ifdef PLUGIN_X264
    vlc_set_description( N_("H.264/MPEG-4 Part 10/AVC encoder (x264)"))
    vlc_set_capability( "video encoder", 200 )
#endif
...
/* Frame-type options */
    ...
    vlc_add_integer( SOUT_CFG_PREFIX "keyint", 250, KEYINT_TEXT,
                 KEYINT_LONGTEXT )

    vlc_add_integer( SOUT_CFG_PREFIX "min-keyint", 25, MIN_KEYINT_TEXT,
                 MIN_KEYINT_LONGTEXT )

    vlc_add_bool( SOUT_CFG_PREFIX "opengop", false, OPENGOP_TEXT,
               OPENGOP_LONGTEXT )
    vlc_add_bool( SOUT_CFG_PREFIX "bluray-compat", false, BLURAY_TEXT,
               BLURAY_LONGTEXT )

    vlc_add_integer( SOUT_CFG_PREFIX "scenecut", 40, SCENE_TEXT,
                 SCENE_LONGTEXT )
        vlc_change_integer_range( -1, 100 )

    vlc_add_integer( SOUT_CFG_PREFIX "bframes", 3, BFRAMES_TEXT,
                 BFRAMES_LONGTEXT )
        vlc_change_integer_range( 0, 16 )

    vlc_add_integer( SOUT_CFG_PREFIX "b-adapt", 1, B_ADAPT_TEXT,
                 B_ADAPT_LONGTEXT )
        vlc_change_integer_range( 0, 2 )

    vlc_add_integer( SOUT_CFG_PREFIX "b-bias", 0, B_BIAS_TEXT,
                 B_BIAS_LONGTEXT )
        vlc_change_integer_range( -100, 100 )

    vlc_add_string( SOUT_CFG_PREFIX "bpyramid", "normal", BPYRAMID_TEXT,
              BPYRAMID_LONGTEXT )
        vlc_change_string_list( bpyramid_list, bpyramid_list_text )

    vlc_add_bool( SOUT_CFG_PREFIX "cabac", true, CABAC_TEXT, CABAC_LONGTEXT )

    vlc_add_bool( SOUT_CFG_PREFIX "fullrange", false, FULLRANGE_TEXT, FULLRANGE_LONGTEXT )

    vlc_add_integer( SOUT_CFG_PREFIX "ref", 3, REF_TEXT,
                 REF_LONGTEXT )
        vlc_change_integer_range( 1, 16 )

    vlc_add_bool( SOUT_CFG_PREFIX "nf", false, NF_TEXT,
              NF_LONGTEXT )

    vlc_add_string( SOUT_CFG_PREFIX "deblock", "0:0", FILTER_TEXT,
                 FILTER_LONGTEXT )

    vlc_add_string( SOUT_CFG_PREFIX "psy-rd", "1.0:0.0", PSY_RD_TEXT,
                PSY_RD_LONGTEXT )

    vlc_add_bool( SOUT_CFG_PREFIX "psy", true, PSY_TEXT, PSY_LONGTEXT )

    vlc_add_string( SOUT_CFG_PREFIX "level", "0", LEVEL_TEXT,
               LEVEL_LONGTEXT )

    vlc_add_string( SOUT_CFG_PREFIX "profile", "high", PROFILE_TEXT,
               PROFILE_LONGTEXT )
    ...
    vlc_add_bool( SOUT_CFG_PREFIX "interlaced", false, INTERLACED_TEXT, INTERLACED_LONGTEXT )

    vlc_add_integer( SOUT_CFG_PREFIX "frame-packing", -1, FRAMEPACKING_TEXT, FRAMEPACKING_LONGTEXT )
        vlc_change_integer_list( framepacking_list, framepacking_list_text )
        vlc_change_integer_range( -1, 6)

    vlc_add_integer( SOUT_CFG_PREFIX "slices", 0, SLICE_COUNT, SLICE_COUNT_LONGTEXT )
    vlc_add_integer( SOUT_CFG_PREFIX "slice-max-size", 0, SLICE_MAX_SIZE, SLICE_MAX_SIZE_LONGTEXT )
    vlc_add_integer( SOUT_CFG_PREFIX "slice-max-mbs", 0, SLICE_MAX_MBS, SLICE_MAX_MBS_LONGTEXT )

    vlc_add_string( SOUT_CFG_PREFIX "hrd", "none", HRD_TEXT, HRD_TEXT )
        vlc_config_set (VLC_CONFIG_LIST,
...
/* Ratecontrol */
    ...
    vlc_add_integer( SOUT_CFG_PREFIX "qp", -1, QP_TEXT, QP_LONGTEXT )
        vlc_change_integer_range( -1, 51 ) /* QP 0 -> lossless encoding */

    vlc_add_integer( SOUT_CFG_PREFIX "crf", 23, CRF_TEXT,
                 CRF_LONGTEXT )
        vlc_change_integer_range( 0, 51 )

    vlc_add_integer( SOUT_CFG_PREFIX "qpmin", 10, QPMIN_TEXT,
                 QPMIN_LONGTEXT )
        vlc_change_integer_range( 0, 51 )

    vlc_add_integer( SOUT_CFG_PREFIX "qpmax", 51, QPMAX_TEXT,
                 QPMAX_LONGTEXT )
        vlc_change_integer_range( 0, 51 )

    vlc_add_integer( SOUT_CFG_PREFIX "qpstep", 4, QPSTEP_TEXT,
                 QPSTEP_LONGTEXT )
        vlc_change_integer_range( 0, 51 )

    vlc_add_float( SOUT_CFG_PREFIX "ratetol", 1.0, RATETOL_TEXT,
               RATETOL_LONGTEXT )
        vlc_change_float_range( 0, 100 )

    vlc_add_integer( SOUT_CFG_PREFIX "vbv-maxrate", 0, VBV_MAXRATE_TEXT,
                 VBV_MAXRATE_LONGTEXT )

    vlc_add_integer( SOUT_CFG_PREFIX "vbv-bufsize", 0, VBV_BUFSIZE_TEXT,
                 VBV_BUFSIZE_LONGTEXT )

    vlc_add_float( SOUT_CFG_PREFIX "vbv-init", 0.9, VBV_INIT_TEXT,
               VBV_INIT_LONGTEXT )
        vlc_change_float_range( 0, 1 )

    vlc_add_float( SOUT_CFG_PREFIX "ipratio", 1.40, IPRATIO_TEXT,
               IPRATIO_LONGTEXT )
        vlc_change_float_range( 1, 2 )

    vlc_add_float( SOUT_CFG_PREFIX "pbratio", 1.30, PBRATIO_TEXT,
               PBRATIO_LONGTEXT )
        vlc_change_float_range( 1, 2 )

    vlc_add_integer( SOUT_CFG_PREFIX "chroma-qp-offset", 0, CHROMA_QP_OFFSET_TEXT,
                 NULL )

    vlc_add_integer( SOUT_CFG_PREFIX "pass", 0, PASS_TEXT,
                 PASS_LONGTEXT )
        vlc_change_integer_range( 0, 3 )

    vlc_add_float( SOUT_CFG_PREFIX "qcomp", 0.60, QCOMP_TEXT,
               QCOMP_LONGTEXT )
        vlc_change_float_range( 0, 1 )

    vlc_add_float( SOUT_CFG_PREFIX "cplxblur", 20.0, CPLXBLUR_TEXT,
               CPLXBLUR_LONGTEXT )

    vlc_add_float( SOUT_CFG_PREFIX "qblur", 0.5, QBLUR_TEXT,
               QBLUR_LONGTEXT )

    vlc_add_integer( SOUT_CFG_PREFIX "aq-mode", X264_AQ_VARIANCE, AQ_MODE_TEXT,
                 AQ_MODE_LONGTEXT )
         vlc_change_integer_range( 0, 2 )
    vlc_add_float( SOUT_CFG_PREFIX "aq-strength", 1.0, AQ_STRENGTH_TEXT,
               AQ_STRENGTH_LONGTEXT )

/* Analysis */

    /* x264 partitions = none (default). set at least "normal" mode. */
    vlc_add_string( SOUT_CFG_PREFIX "partitions", "normal", ANALYSE_TEXT,
                ANALYSE_LONGTEXT )
        vlc_change_string_list( enc_analyse_list, enc_analyse_list_text )

    vlc_add_string( SOUT_CFG_PREFIX "direct", "spatial", DIRECT_PRED_TEXT,
                NULL )
        vlc_change_string_list( direct_pred_list, direct_pred_list_text )

    vlc_add_integer( SOUT_CFG_PREFIX "direct-8x8", 1, DIRECT_PRED_SIZE_TEXT,
                 DIRECT_PRED_SIZE_LONGTEXT )
        vlc_change_integer_range( -1, 1 )

    vlc_add_bool( SOUT_CFG_PREFIX "weightb", true, WEIGHTB_TEXT,
              NULL )

    vlc_add_integer( SOUT_CFG_PREFIX "weightp", 2, WEIGHTP_TEXT,
              WEIGHTP_LONGTEXT )
        vlc_change_integer_range( 0, 2 )

    vlc_add_string( SOUT_CFG_PREFIX "me", "hex", ME_TEXT,
                ME_LONGTEXT )
        vlc_change_string_list( enc_me_list, enc_me_list_text )

    vlc_add_integer( SOUT_CFG_PREFIX "merange", 16, MERANGE_TEXT,
                 MERANGE_LONGTEXT )
        vlc_change_integer_range( 1, 64 )

    vlc_add_integer( SOUT_CFG_PREFIX "mvrange", -1, MVRANGE_TEXT,
                 MVRANGE_LONGTEXT )

    vlc_add_integer( SOUT_CFG_PREFIX "mvrange-thread", -1, MVRANGE_THREAD_TEXT,
                 MVRANGE_THREAD_LONGTEXT )

    vlc_add_integer( SOUT_CFG_PREFIX "subme", 7, SUBME_TEXT,
                 SUBME_LONGTEXT )

    vlc_add_bool( SOUT_CFG_PREFIX "mixed-refs", true, MIXED_REFS_TEXT,
              MIXED_REFS_LONGTEXT )

    vlc_add_bool( SOUT_CFG_PREFIX "chroma-me", true, CHROMA_ME_TEXT,
              CHROMA_ME_LONGTEXT )

    vlc_add_bool( SOUT_CFG_PREFIX "8x8dct", true, TRANSFORM_8X8DCT_TEXT,
              TRANSFORM_8X8DCT_LONGTEXT )

    vlc_add_integer( SOUT_CFG_PREFIX "trellis", 1, TRELLIS_TEXT,
                 TRELLIS_LONGTEXT )
        vlc_change_integer_range( 0, 2 )

    vlc_add_integer( SOUT_CFG_PREFIX "lookahead", 40, LOOKAHEAD_TEXT,
                 LOOKAHEAD_LONGTEXT )
        vlc_change_integer_range( 0, 60 )

    vlc_add_bool( SOUT_CFG_PREFIX "intra-refresh", false, INTRAREFRESH_TEXT,
              INTRAREFRESH_LONGTEXT )

    vlc_add_bool( SOUT_CFG_PREFIX "mbtree", true, MBTREE_TEXT, MBTREE_LONGTEXT )

    vlc_add_bool( SOUT_CFG_PREFIX "fast-pskip", true, FAST_PSKIP_TEXT,
              NULL )

    vlc_add_bool( SOUT_CFG_PREFIX "dct-decimate", true, DCT_DECIMATE_TEXT,
              DCT_DECIMATE_LONGTEXT )

    vlc_add_integer( SOUT_CFG_PREFIX "nr", 0, NR_TEXT,
                 NR_LONGTEXT )
        vlc_change_integer_range( 0, 1000 )

    vlc_add_integer( SOUT_CFG_PREFIX "deadzone-inter", 21, DEADZONE_INTER_TEXT,
                 DEADZONE_INTER_LONGTEXT )
        vlc_change_integer_range( 0, 32 )

    vlc_add_integer( SOUT_CFG_PREFIX "deadzone-intra", 11, DEADZONE_INTRA_TEXT,
                 DEADZONE_INTRA_LONGTEXT )
        vlc_change_integer_range( 0, 32 )

/* Input/Output */

    vlc_add_bool( SOUT_CFG_PREFIX "non-deterministic", false, NON_DETERMINISTIC_TEXT,
              NON_DETERMINISTIC_LONGTEXT )

    vlc_add_bool( SOUT_CFG_PREFIX "asm", true, ASM_TEXT,
              ASM_LONGTEXT )

    /* x264 psnr = 1 (default). disable PSNR computation for speed. */
    vlc_add_bool( SOUT_CFG_PREFIX "psnr", false, PSNR_TEXT,
              PSNR_LONGTEXT )

    /* x264 ssim = 1 (default). disable SSIM computation for speed. */
    vlc_add_bool( SOUT_CFG_PREFIX "ssim", false, SSIM_TEXT,
              SSIM_LONGTEXT )

    vlc_add_bool( SOUT_CFG_PREFIX "quiet", false, QUIET_TEXT,
              NULL )

    vlc_add_integer( SOUT_CFG_PREFIX "sps-id", 0, SPS_ID_TEXT,
                 SPS_ID_LONGTEXT )

    vlc_add_bool( SOUT_CFG_PREFIX "aud", false, AUD_TEXT,
              AUD_LONGTEXT )

    vlc_add_bool( SOUT_CFG_PREFIX "verbose", false, VERBOSE_TEXT,
              VERBOSE_LONGTEXT )

    vlc_add_string( SOUT_CFG_PREFIX "stats", "x264_2pass.log", STATS_TEXT,
                STATS_LONGTEXT )

    vlc_add_string( SOUT_CFG_PREFIX "preset", NULL , PRESET_TEXT , NULL )
        vlc_config_set (VLC_CONFIG_LIST,
            (sizeof(x264_preset_names) / sizeof (char*)) - 1,
            x264_preset_names, x264_preset_names);
    vlc_add_string( SOUT_CFG_PREFIX "tune", NULL , TUNE_TEXT, NULL )
        vlc_config_set (VLC_CONFIG_LIST,
            (sizeof(x264_tune_names) / sizeof (char*)) - 1,
            x264_tune_names, x264_tune_names);

    vlc_add_string( SOUT_CFG_PREFIX "options", NULL, X264_OPTIONS_TEXT,
