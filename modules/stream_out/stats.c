...
vlc_module_begin()
    ...
    vlc_set_description( N_("Writes statistic info about stream"))
    vlc_set_capability( "sout output", 0 )
    vlc_add_shortcut( "stats" )
    vlc_set_subcategory( SUBCAT_SOUT_STREAM )
    vlc_set_callback( OutputOpen )
    vlc_add_string( SOUT_CFG_PREFIX "output", "", OUTPUT_TEXT,OUTPUT_LONGTEXT )
    vlc_add_string( SOUT_CFG_PREFIX "prefix", "stats", PREFIX_TEXT, NULL )
    vlc_add_submodule()
    vlc_set_capability( "sout filter", 0 )
    vlc_add_shortcut( "stats" )
    vlc_set_callback( FilterOpen )
vlc_module_end()
