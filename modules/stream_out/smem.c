...
vlc_module_begin ()
    vlc_set_shortname( N_("Smem"))
    vlc_set_description( N_("Stream output to memory buffer") )
    vlc_set_capability( "sout output", 0 )
    vlc_add_shortcut( "smem" )
    vlc_set_subcategory( SUBCAT_SOUT_STREAM )
    vlc_add_string( SOUT_PREFIX_VIDEO "prerender-callback", "0", T_VIDEO_PRERENDER_CALLBACK, LT_VIDEO_PRERENDER_CALLBACK )
        vlc_change_volatile()
    vlc_add_string( SOUT_PREFIX_AUDIO "prerender-callback", "0", T_AUDIO_PRERENDER_CALLBACK, LT_AUDIO_PRERENDER_CALLBACK )
        vlc_change_volatile()
    vlc_add_string( SOUT_PREFIX_VIDEO "postrender-callback", "0", T_VIDEO_POSTRENDER_CALLBACK, LT_VIDEO_POSTRENDER_CALLBACK )
        vlc_change_volatile()
    vlc_add_string( SOUT_PREFIX_AUDIO "postrender-callback", "0", T_AUDIO_POSTRENDER_CALLBACK, LT_AUDIO_POSTRENDER_CALLBACK )
        vlc_change_volatile()
    vlc_add_string( SOUT_PREFIX_VIDEO "data", "0", T_VIDEO_DATA, LT_VIDEO_DATA )
        vlc_change_volatile()
    vlc_add_string( SOUT_PREFIX_AUDIO "data", "0", T_AUDIO_DATA, LT_VIDEO_DATA )
        vlc_change_volatile()
    vlc_add_bool( SOUT_CFG_PREFIX "time-sync", true, T_TIME_SYNC, LT_TIME_SYNC )
        vlc_change_private()
