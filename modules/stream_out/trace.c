...
vlc_module_begin()
    vlc_set_shortname(N_("Trace"))
    vlc_set_description(N_("Trace frame timestamps and PCR events"))
    vlc_set_capability("sout filter", 0)
    vlc_set_help(HELP_TEXT)
    vlc_add_shortcut("trace")
    vlc_set_callback(Open)

    vlc_add_string(SOUT_CFG_PREFIX "name", NULL, NAME_TEXT, NAME_LONGTEXT)
vlc_module_end()
