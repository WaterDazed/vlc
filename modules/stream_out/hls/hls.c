...
{
    char *mux_config;
    if (asprintf(&mux_config,
                 "avformat{mux=mp4,options{movflags=frag_keyframe+"
                 "delay_moov+dash,frag_duration=%" PRIi64 "}}",
                 US_FROM_VLC_TICK(config->segment_length)) == -1)
        return NULL;

    sout_mux_t *mp4 = sout_MuxNew(access, mux_config);
    free(mux_config);
    return mp4;
}
