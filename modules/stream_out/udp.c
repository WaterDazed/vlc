...
vlc_module_begin()
    vlc_set_shortname(N_("UDP"))
    vlc_set_description(N_("UDP stream output"))
    vlc_set_capability("sout output", 40)
    vlc_add_shortcut("standard", "std", "udp")
    vlc_set_subcategory(SUBCAT_SOUT_STREAM)

    vlc_add_bool(SOUT_CFG_PREFIX "avformat", false, AVF_TEXT, AVF_LONGTEXT)
    vlc_add_string(SOUT_CFG_PREFIX "dst", "", DEST_TEXT, DEST_LONGTEXT)
    vlc_add_bool(SOUT_CFG_PREFIX "sap", false, SAP_TEXT, SAP_LONGTEXT)
    vlc_add_string(SOUT_CFG_PREFIX "name", "", NAME_TEXT, NAME_LONGTEXT)
    vlc_add_string(SOUT_CFG_PREFIX "description", "", DESC_TEXT, DESC_LONGTEXT)
