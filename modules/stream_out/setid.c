...
vlc_module_begin()
    vlc_set_shortname( N_("Set ID"))
    vlc_set_section( N_("Set ES id"), NULL )
    vlc_set_description( N_("Change the id of an elementary stream"))
    vlc_set_capability( "sout filter", 50 )
    vlc_add_shortcut( "setid" )
    vlc_set_subcategory( SUBCAT_SOUT_STREAM )
    vlc_set_callback( OpenId )
    vlc_add_integer( SOUT_CFG_PREFIX_ID "id", 0, ID_TEXT, ID_LONGTEXT )
    vlc_add_integer( SOUT_CFG_PREFIX_ID "new-id", 0, NEWID_TEXT, NEWID_LONGTEXT )

    vlc_add_submodule ()
    vlc_set_section( N_("Set ES Lang"), NULL )
    vlc_set_shortname( N_("Set Lang"))
    vlc_set_description( N_("Change the language of an elementary stream"))
    vlc_set_capability( "sout filter", 50 )
    vlc_add_shortcut( "setlang" )
    vlc_set_callback( OpenLang )
    vlc_add_integer( SOUT_CFG_PREFIX_LANG "id", 0, ID_TEXT, ID_LONGTEXT )
    vlc_add_string( SOUT_CFG_PREFIX_LANG "lang", "eng", LANG_TEXT, LANG_LONGTEXT )
