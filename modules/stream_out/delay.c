...
vlc_module_begin()
    ...
    vlc_set_description(N_("Delay a stream"))
    vlc_set_capability("sout filter", 50)
    vlc_add_shortcut("delay")
    vlc_set_subcategory(SUBCAT_SOUT_STREAM)
    vlc_set_callback(Open)
    vlc_add_integer(SOUT_CFG_PREFIX "id", 0, ID_TEXT, ID_LONGTEXT)
    vlc_add_integer(SOUT_CFG_PREFIX "delay", 0, DELAY_TEXT, DELAY_LONGTEXT)
vlc_module_end()
