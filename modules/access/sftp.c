...
vlc_module_begin ()
    vlc_set_shortname( "SFTP" )
    vlc_set_description( N_("SFTP input") )
    vlc_set_capability( "access", 0 )
    vlc_set_subcategory( SUBCAT_INPUT_ACCESS )
    vlc_add_integer( "sftp-port", 22, PORT_TEXT, PORT_LONGTEXT )
    vlc_add_string( "sftp-user", NULL, USER_TEXT, USER_LONGTEXT )
    vlc_add_password("sftp-pwd", NULL, PASS_TEXT, PASS_LONGTEXT)
    vlc_add_loadfile("sftp-privatekey", NULL, PRIVATEKEY_TEXT, PRIVATEKEY_LONGTEXT)
    vlc_add_shortcut( "sftp" )
