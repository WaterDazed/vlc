...
vlc_module_begin()
    vlc_set_shortname(N_("WASAPI"))
    vlc_set_description(N_("Windows Audio Session API input"))
    vlc_set_capability("access", 0)
    vlc_set_subcategory(SUBCAT_INPUT_ACCESS)

    vlc_add_bool("wasapi-loopback", false, LOOPBACK_TEXT, LOOPBACK_LONGTEXT)

    vlc_add_shortcut("wasapi")
