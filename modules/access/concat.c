...
vlc_module_begin()
    vlc_set_shortname(N_("Concatenation"))
    vlc_set_description(N_("Concatenated inputs"))
    vlc_set_subcategory(SUBCAT_INPUT_ACCESS)
    vlc_add_string("concat-list", NULL, INPUT_LIST_TEXT, INPUT_LIST_LONGTEXT)
    vlc_set_capability("access", 0)
    vlc_set_callbacks(Open, Close)
    vlc_add_shortcut("concat", "list")
vlc_module_end()
