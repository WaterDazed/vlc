...
vlc_module_begin()
    vlc_set_shortname(N_("Memory input"))
    vlc_set_description(N_("Memory input"))
    vlc_set_subcategory(SUBCAT_INPUT_ACCESS)

    vlc_add_string ("imem-get", "0", GET_TEXT, GET_LONGTEXT)
        vlc_change_volatile()
    vlc_add_string ("imem-release", "0", RELEASE_TEXT, RELEASE_LONGTEXT)
        vlc_change_volatile()
    vlc_add_string ("imem-cookie", NULL, COOKIE_TEXT, COOKIE_LONGTEXT)
        vlc_change_volatile()
        vlc_change_safe()
    vlc_add_string ("imem-data", "0", DATA_TEXT, DATA_LONGTEXT)
        vlc_change_volatile()

    vlc_add_integer("imem-id", -1, ID_TEXT, ID_LONGTEXT)
        vlc_change_private()
        vlc_change_safe()
    vlc_add_integer("imem-group", 0, GROUP_TEXT, GROUP_LONGTEXT)
        vlc_change_private()
        vlc_change_safe()
    vlc_add_integer("imem-cat", 0, CAT_TEXT, CAT_LONGTEXT)
        vlc_change_integer_list(cat_values, cat_texts)
        vlc_change_private()
        vlc_change_safe()
    vlc_add_string ("imem-codec", NULL, CODEC_TEXT, CODEC_LONGTEXT)
        vlc_change_private()
        vlc_change_safe()
    vlc_add_string( "imem-language", NULL, LANGUAGE_TEXT, LANGUAGE_LONGTEXT)
        vlc_change_private()
        vlc_change_safe()

    vlc_add_integer("imem-samplerate", 0, SAMPLERATE_TEXT, SAMPLERATE_LONGTEXT)
        vlc_change_private()
        vlc_change_safe()
    vlc_add_integer("imem-channels", 0, CHANNELS_TEXT, CHANNELS_LONGTEXT)
        vlc_change_private()
        vlc_change_safe()

    vlc_add_integer_with_range("imem-width", 0, 0, UINT_MAX, WIDTH_TEXT, WIDTH_LONGTEXT)
        vlc_change_private()
        vlc_change_safe()
    vlc_add_integer_with_range("imem-height", 0, 0, UINT_MAX, HEIGHT_TEXT, HEIGHT_LONGTEXT)
        vlc_change_private()
        vlc_change_safe()
    vlc_add_string ("imem-dar", NULL, DAR_TEXT, DAR_LONGTEXT)
        vlc_change_private()
        vlc_change_safe()
    vlc_add_string ("imem-fps", NULL, FPS_TEXT, FPS_LONGTEXT)
        vlc_change_private()
        vlc_change_safe()

    vlc_add_integer ("imem-size", 0, SIZE_TEXT, SIZE_LONGTEXT)
        vlc_change_private()
        vlc_change_safe()

    vlc_add_shortcut("imem")
    vlc_set_capability("access", 1)
    vlc_set_callbacks(OpenDemux, CloseDemux)

    vlc_add_submodule()
        vlc_add_shortcut("imem")
        vlc_set_capability("access", 0)
        vlc_set_callbacks(OpenAccess, CloseAccess)
vlc_module_end()
