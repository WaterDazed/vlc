...
vlc_module_begin ()
    vlc_set_shortname( "dsm" )
    vlc_set_description( N_("libdsm SMB input") )
    vlc_set_help(BDSM_HELP)
    vlc_set_subcategory( SUBCAT_INPUT_ACCESS )
    vlc_add_string( "smb-user", NULL, SMB_USER_TEXT, SMB_USER_LONGTEXT )
    vlc_add_password("smb-pwd", NULL, SMB_PASS_TEXT, SMB_PASS_LONGTEXT)
    vlc_add_string( "smb-domain", NULL, SMB_DOMAIN_TEXT, SMB_DOMAIN_LONGTEXT )
    vlc_add_bool( "smb-force-v1", false, SMB_FORCE_V1_TEXT, SMB_FORCE_V1_LONGTEXT )
    vlc_add_shortcut( "smb", "cifs" )

    vlc_set_capability( "access", 22 )
    vlc_set_callbacks( OpenForced, Close )

    vlc_add_submodule()
        vlc_set_capability( "access", 20 )
        vlc_set_callbacks( OpenNotForced, Close )
        vlc_add_shortcut( "smb", "cifs" )

    vlc_add_submodule()
        vlc_add_shortcut( "dsm-sd" )
        vlc_set_description( N_("libdsm NETBIOS discovery module") )
        vlc_set_subcategory( SUBCAT_PLAYLIST_SD )
        vlc_set_capability( "services_discovery", 0 )
        vlc_set_callbacks( bdsm_SdOpen, bdsm_SdClose )
