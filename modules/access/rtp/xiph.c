...
vlc_module_begin()
    vlc_set_shortname(N_("RTP Xiph"))
    vlc_set_description(N_("RTP Xiph payload parser"))
    vlc_set_subcategory(SUBCAT_INPUT_DEMUX)
    vlc_set_capability("rtp audio parser", 0)
    vlc_set_callback(vorbis_open)
    vlc_add_shortcut("vorbis")

    vlc_add_submodule()
    vlc_set_capability("rtp video parser", 0)
    vlc_set_callback(theora_open)
