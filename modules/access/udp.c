...
vlc_module_begin()
    ...
    vlc_set_description(N_("UDP input"))
    vlc_set_subcategory(SUBCAT_INPUT_ACCESS)

    vlc_add_obsolete_integer("udp-buffer") /* since 3.0.0 */
    vlc_add_integer("udp-timeout", -1, TIMEOUT_TEXT, NULL)

    vlc_set_capability("access", 0)
    vlc_add_shortcut("udp", "udpstream", "udp4", "udp6")
