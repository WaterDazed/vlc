...
vlc_module_begin()
    vlc_set_shortname( N_("VNC") )
    vlc_add_shortcut( "vnc" )
    vlc_set_subcategory( SUBCAT_INPUT_ACCESS )
    vlc_set_description( N_("VNC client access") )
    vlc_set_capability( "access", 0 )

    vlc_add_string( CFG_PREFIX "user", NULL, RFB_USER, NULL )
        vlc_change_safe()
    vlc_add_password(CFG_PREFIX "password", NULL, RFB_PASSWORD, NULL)
        vlc_change_safe()
    vlc_add_loadfile(CFG_PREFIX "x509-ca", NULL, RFB_CA_TEXT, RFB_CA_LONGTEXT)
        vlc_change_safe()
    vlc_add_loadfile(CFG_PREFIX "x509-crl", NULL, RFB_CRL_TEXT, RFB_CRL_LONGTEXT)
        vlc_change_safe()
    vlc_add_loadfile(CFG_PREFIX "x509-client-cert", NULL,
                 RFB_CERT_TEXT, RFB_CERT_LONGTEXT)
        vlc_change_safe()
    vlc_add_loadfile(CFG_PREFIX "x509-client-key", NULL,
                 RFB_KEY_TEXT, RFB_KEY_LONGTEXT)
        vlc_change_safe()

    vlc_add_float( CFG_PREFIX "fps", 5, RFB_FPS, RFB_FPS_LONGTEXT )
    vlc_add_string( CFG_PREFIX "chroma", rgb_chromas_v[0], RFB_CHROMA, RFB_CHROMA_LONGTEXT )
        vlc_change_string_list (rgb_chromas_v, rgb_chromas)
        vlc_change_safe()
    vlc_add_integer( CFG_PREFIX "compress-level", 0, RFB_COMPRESS, RFB_COMPRESS_LONGTEXT )
        vlc_change_integer_range (0, 9)
        vlc_change_safe()
    vlc_add_integer( CFG_PREFIX "quality-level", 9, RFB_QUALITY, RFB_QUALITY_LONGTEXT )
