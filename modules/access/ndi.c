 ...
 *****************************************************************************
 * Copyright © 2025 VideoLAN, VLC authors, and libndi authors
 *
...
static int data_callback( ndi_packet_t *ndi_packet, void *user_data )
{
    ...
    switch ( ndi_packet->type )
    {
        ...
        {
            ndi_packet_metadata_t *meta = (ndi_packet_metadata_t *)ndi_packet->packet;
            msg_Dbg( p_demux, "Received NDI metadata: %s", meta->data );
            break;
        }

        default:
            msg_Warn( p_demux, "Received unsupported NDI data type: %d", ndi_packet->type );
            return -1;
    }
...
static int Demux( demux_t *p_demux )
{
    ...
    struct pollfd fd;
    fd.fd = ndi_recv_get_socket_fd( p_sys->ndi_ctx );
    fd.events = ndi_recv_which_events( p_sys->ndi_ctx );

    if ( !ndi_recv_is_connected( p_sys->ndi_ctx ) )
    ...
    while ( ndi_recv_capture( p_sys->ndi_ctx ) < 0 && !vlc_killed() )
    {
        if ( errno != EAGAIN )
        {
...
static int Open( vlc_object_t *p_this )
{
    ...
    ndi_recv_ctx_t ndi_ctx = ndi_recv_create( &opts, data_callback, p_demux, 1 );
    ...
    while ( ndi_recv_connect( ndi_ctx ) < 0 )
    {
        ...
        fd.fd = ndi_recv_get_socket_fd( ndi_ctx );
        fd.events = ndi_recv_which_events( ndi_ctx );
    ...
    while ( ndi_recv_send_metadata( ndi_ctx ) < 0 )
    {
        if ( errno != EAGAIN )
        {
