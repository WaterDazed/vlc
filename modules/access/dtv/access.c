...
vlc_module_begin ()
    vlc_set_shortname (N_("DTV"))
    vlc_set_description (N_("Digital Television and Radio"))
    vlc_set_subcategory (SUBCAT_INPUT_ACCESS)
    vlc_set_capability ("access", 0)
    vlc_set_callbacks (Open, Close)
    vlc_add_shortcut ("dtv", "tv", "dvb", /* "radio", "dab",*/
...
#ifdef HAVE_LINUX_DVB
    vlc_add_integer ("dvb-adapter", 0, ADAPTER_TEXT, ADAPTER_LONGTEXT)
        vlc_change_integer_range (0, 255)
        vlc_change_safe ()
    vlc_add_integer ("dvb-device", 0, DEVICE_TEXT, DEVICE_LONGTEXT)
        vlc_change_integer_range (0, 255)
        vlc_change_safe ()
    vlc_add_bool ("dvb-budget-mode", false, BUDGET_TEXT, BUDGET_LONGTEXT)
#endif
#ifdef _WIN32
    vlc_add_integer ("dvb-adapter", -1, ADAPTER_TEXT, ADAPTER_LONGTEXT)
        vlc_change_safe ()
    vlc_add_string ("dvb-network-name", "", NAME_TEXT, NAME_LONGTEXT)
    /* Hmm: is this one really safe??: */
    vlc_add_string ("dvb-create-name", "", CREATE_TEXT, CREATE_LONGTEXT)
        vlc_change_private ()
#endif
    vlc_add_integer ("dvb-frequency", 0, FREQ_TEXT, FREQ_LONGTEXT)
        vlc_change_integer_range (0, 107999999)
        vlc_change_safe ()
    vlc_add_integer ("dvb-inversion", -1, INVERSION_TEXT, INVERSION_LONGTEXT)
        vlc_change_integer_list (auto_off_on_vlc, auto_off_on_user)
        vlc_change_safe ()

    vlc_set_section (N_("Terrestrial reception parameters"), NULL)
    vlc_add_integer ("dvb-bandwidth", 0, BANDWIDTH_TEXT, NULL)
        vlc_change_integer_list (bandwidth_vlc, bandwidth_user)
        vlc_change_safe ()
    vlc_add_integer ("dvb-transmission", -1,
                 TRANSMISSION_TEXT, NULL)
        vlc_change_integer_list (transmission_vlc, transmission_user)
        vlc_change_safe ()
    vlc_add_string ("dvb-guard", "", GUARD_TEXT, NULL)
        vlc_change_string_list (guard_vlc, guard_user)
        vlc_change_safe ()

    vlc_set_section (N_("DVB-T reception parameters"), NULL)
    vlc_add_string ("dvb-code-rate-hp", "",
                CODE_RATE_HP_TEXT, CODE_RATE_LONGTEXT)
        vlc_change_string_list (code_rate_vlc, code_rate_user)
        vlc_change_safe ()
    vlc_add_string ("dvb-code-rate-lp", "",
                CODE_RATE_LP_TEXT, CODE_RATE_LONGTEXT)
        vlc_change_string_list (code_rate_vlc, code_rate_user)
        vlc_change_safe ()
    vlc_add_integer ("dvb-hierarchy", -1, HIERARCHY_TEXT, NULL)
        vlc_change_integer_list (hierarchy_vlc, hierarchy_user)
        vlc_change_safe ()
    vlc_add_integer ("dvb-plp-id", 0, PLP_ID_TEXT, NULL)
        vlc_change_integer_range (0, 255)
        vlc_change_safe ()

    vlc_set_section (N_("ISDB-T reception parameters"), NULL)
    vlc_add_string ("dvb-a-modulation", NULL,
                MODULATION_A_TEXT, MODULATION_LONGTEXT)
        vlc_change_string_list (modulation_vlc, modulation_user)
        vlc_change_safe ()
    vlc_add_string ("dvb-a-fec", NULL, CODE_RATE_A_TEXT, CODE_RATE_LONGTEXT)
        vlc_change_string_list (code_rate_vlc, code_rate_user)
        vlc_change_safe ()
    vlc_add_integer ("dvb-a-count", 0, SEGMENT_COUNT_A_TEXT, NULL)
        vlc_change_integer_range (0, 13)
        vlc_change_safe ()
    vlc_add_integer ("dvb-a-interleaving", 0, TIME_INTERLEAVING_A_TEXT, NULL)
        vlc_change_integer_range (0, 3)
        vlc_change_safe ()
    vlc_add_string ("dvb-b-modulation", NULL,
                MODULATION_B_TEXT, MODULATION_LONGTEXT)
        vlc_change_string_list (modulation_vlc, modulation_user)
        vlc_change_safe ()
    vlc_add_string ("dvb-b-fec", NULL, CODE_RATE_B_TEXT, CODE_RATE_LONGTEXT)
        vlc_change_string_list (code_rate_vlc, code_rate_user)
        vlc_change_safe ()
    vlc_add_integer ("dvb-b-count", 0, SEGMENT_COUNT_B_TEXT, NULL)
        vlc_change_integer_range (0, 13)
        vlc_change_safe ()
    vlc_add_integer ("dvb-b-interleaving", 0, TIME_INTERLEAVING_B_TEXT, NULL)
        vlc_change_integer_range (0, 3)
        vlc_change_safe ()
    vlc_add_string ("dvb-c-modulation", NULL,
                MODULATION_C_TEXT, MODULATION_LONGTEXT)
        vlc_change_string_list (modulation_vlc, modulation_user)
        vlc_change_safe ()
    vlc_add_string ("dvb-c-fec", NULL, CODE_RATE_C_TEXT, CODE_RATE_LONGTEXT)
        vlc_change_string_list (code_rate_vlc, code_rate_user)
        vlc_change_safe ()
    vlc_add_integer ("dvb-c-count", 0, SEGMENT_COUNT_C_TEXT, NULL)
        vlc_change_integer_range (0, 13)
        vlc_change_safe ()
    vlc_add_integer ("dvb-c-interleaving", 0, TIME_INTERLEAVING_C_TEXT, NULL)
        vlc_change_integer_range (0, 3)
        vlc_change_safe ()

    vlc_set_section (N_("Cable and satellite reception parameters"), NULL)
    vlc_add_string ("dvb-modulation", NULL,
                 MODULATION_TEXT, MODULATION_LONGTEXT)
        vlc_change_string_list (modulation_vlc, modulation_user)
        vlc_change_safe ()
    vlc_add_integer ("dvb-srate", 0, SRATE_TEXT, SRATE_LONGTEXT)
        vlc_change_integer_range (0, UINT64_C(0xffffffff))
        vlc_change_safe ()
    vlc_add_string ("dvb-fec", "", CODE_RATE_TEXT, CODE_RATE_LONGTEXT)
        vlc_change_string_list (code_rate_vlc, code_rate_user)
        vlc_change_safe ()

    vlc_set_section (N_("DVB-S2 parameters"), NULL)
    vlc_add_integer ("dvb-stream", 0, STREAM_ID_TEXT, NULL)
        vlc_change_integer_range (0, 255)
        vlc_change_safe ()
    vlc_add_integer ("dvb-pilot", -1, PILOT_TEXT, NULL)
        vlc_change_integer_list (auto_off_on_vlc, auto_off_on_user)
        vlc_change_safe ()
    vlc_add_integer ("dvb-rolloff", -1, ROLLOFF_TEXT, NULL)
        vlc_change_integer_list (rolloff_vlc, rolloff_user)
        vlc_change_safe ()

    vlc_set_section (N_("ISDB-S parameters"), NULL)
    vlc_add_integer ("dvb-ts-id", 0, TS_ID_TEXT, NULL)
        vlc_change_integer_range (0, 0xffff)
        vlc_change_safe ()

    vlc_set_section (N_("Satellite equipment control"), NULL)
    vlc_add_string ("dvb-polarization", "",
                POLARIZATION_TEXT, POLARIZATION_LONGTEXT)
        vlc_change_string_list (polarization_vlc, polarization_user)
        vlc_change_safe ()
    vlc_add_obsolete_integer("dvb-voltage") /* deprecated in 1.2, removed in 4.0 */
#ifdef HAVE_LINUX_DVB
    vlc_add_bool ("dvb-high-voltage", false,
              HIGH_VOLTAGE_TEXT, HIGH_VOLTAGE_LONGTEXT)
#endif
    vlc_add_integer ("dvb-lnb-low", 0, LNB_LOW_TEXT, LNB_LONGTEXT)
        vlc_change_integer_range (0, 0x7fffffff)
    vlc_add_integer ("dvb-lnb-high", 0, LNB_HIGH_TEXT, LNB_LONGTEXT)
        vlc_change_integer_range (0, 0x7fffffff)
    vlc_add_integer ("dvb-lnb-switch", 11700000,
                 LNB_SWITCH_TEXT, LNB_SWITCH_LONGTEXT)
        vlc_change_integer_range (0, 0x7fffffff)
#ifdef HAVE_LINUX_DVB
    vlc_add_integer ("dvb-satno", 0, SATNO_TEXT, SATNO_LONGTEXT)
        vlc_change_integer_list (satno_vlc, satno_user)
        vlc_change_safe ()
    vlc_add_integer ("dvb-uncommitted", 0, UNCOMMITTED_TEXT, UNCOMMITTED_LONGTEXT)
        vlc_change_integer_list (satno_vlc, satno_user)
        vlc_change_safe ()
    vlc_add_integer ("dvb-tone", -1, TONE_TEXT, TONE_LONGTEXT)
        vlc_change_integer_list (auto_off_on_vlc, auto_off_on_user)
#endif
#ifdef _WIN32
    vlc_add_integer ("dvb-network-id", 0, NETID_TEXT, NULL)
    vlc_add_integer ("dvb-azimuth", 0, AZIMUTH_TEXT, AZIMUTH_LONGTEXT)
    vlc_add_integer ("dvb-elevation", 0, ELEVATION_TEXT, ELEVATION_LONGTEXT)
    vlc_add_integer ("dvb-longitude", 0, LONGITUDE_TEXT, LONGITUDE_LONGTEXT)
    vlc_add_string ("dvb-range", "", RANGE_TEXT, RANGE_LONGTEXT)
    /* dvb-range corresponds to the BDA InputRange parameter which is
    * used by some drivers to control the diseqc */

    vlc_set_section (N_("ATSC reception parameters"), NULL)
    vlc_add_integer ("dvb-major-channel", 0, MAJOR_CHANNEL_TEXT, NULL)
    vlc_add_integer ("dvb-minor-channel", 0, MINOR_CHANNEL_TEXT, NULL)
    vlc_add_integer ("dvb-physical-channel", 0, PHYSICAL_CHANNEL_TEXT, NULL)
#endif
vlc_module_end ()
