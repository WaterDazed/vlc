...
vlc_module_begin ()
        vlc_set_shortname( N_("Lua") )
        vlc_set_description( N_("Lua interpreter") )
        vlc_set_subcategory( SUBCAT_INTERFACE_MAIN )

        vlc_add_bool( "lua", true, LUA_TEXT, NULL )
        vlc_add_string( "lua-intf", "dummy", INTF_TEXT, INTF_LONGTEXT )
        vlc_add_string( "lua-config", "", CONFIG_TEXT, CONFIG_LONGTEXT )
        vlc_set_capability( "interface", 0 )
        vlc_set_callbacks( Open_LuaIntf, Close_LuaIntf )
        vlc_add_shortcut( "luaintf" )

    vlc_add_submodule ()
        vlc_set_section( N_("Lua HTTP"), 0 )
            vlc_add_password("http-password", NULL, PASS_TEXT, PASS_LONGTEXT)
            vlc_add_string ( "http-src",  NULL, SRC_TEXT,  SRC_LONGTEXT )
            vlc_add_bool   ( "http-index", false, INDEX_TEXT, INDEX_LONGTEXT )
        vlc_set_capability( "interface", 0 )
        vlc_set_callbacks( Open_LuaHTTP, Close_LuaIntf )
        vlc_add_shortcut( "luahttp", "http" )
        vlc_set_description( N_("Lua HTTP") )

    vlc_add_submodule ()
        vlc_set_section( N_("Lua Telnet"), 0 )
            vlc_add_string( "telnet-host", "localhost", TELNETHOST_TEXT,
                        TELNETHOST_LONGTEXT )
            vlc_add_integer( "telnet-port", TELNETPORT_DEFAULT, TELNETPORT_TEXT,
                         TELNETPORT_LONGTEXT )
                vlc_change_integer_range( 1, 65535 )
            vlc_add_password("telnet-password", NULL, TELNETPWD_TEXT,
                         TELNETPWD_LONGTEXT)
        vlc_set_capability( "interface", 0 )
        vlc_set_callbacks( Open_LuaTelnet, Close_LuaIntf )
        vlc_set_description( N_("Lua Telnet") )
        vlc_add_shortcut( "luatelnet", "telnet" )

    vlc_add_submodule ()
        vlc_set_shortname( N_( "Lua Meta Fetcher" ) )
        vlc_set_description( N_("Fetch meta data using lua scripts") )
        vlc_set_capability( "meta fetcher", 10 )
        vlc_set_callback( FetchMeta )

    vlc_add_submodule ()
        vlc_set_shortname( N_( "Lua Meta Reader" ) )
        vlc_set_description( N_("Read meta data using lua scripts") )
        vlc_set_capability( "meta reader", 10 )
        vlc_set_callback( ReadMeta )

    vlc_add_submodule ()
        vlc_add_shortcut( "luaplaylist" )
        vlc_set_shortname( N_("Lua Playlist") )
        vlc_set_description( N_("Lua Playlist Parser Interface") )
        vlc_set_capability( "demux", 5 )
        vlc_set_callbacks( Import_LuaPlaylist, Close_LuaPlaylist )

    vlc_add_submodule ()
        vlc_set_shortname( N_( "Lua Art" ) )
        vlc_set_description( N_("Fetch artwork using lua scripts") )
        vlc_set_capability( "art finder", 10 )
        vlc_set_callback( FindArt )

    vlc_add_submodule ()
        vlc_set_shortname( N_("Lua Extension") )
        vlc_set_description( N_("Lua Extension") )
        vlc_add_shortcut( "luaextension" )
        vlc_set_capability( "extension", 1 )
        vlc_set_callbacks( Open_Extension, Close_Extension )

    vlc_add_submodule ()
        vlc_set_description( N_("Lua SD Module") )
        vlc_add_shortcut( "luasd" )
        vlc_set_capability( "services_discovery", 0 )
        vlc_add_string( "lua-sd", "", NULL, NULL )
            vlc_change_volatile()
        vlc_set_callbacks( Open_LuaSD, Close_LuaSD )
