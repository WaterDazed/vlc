 ...
 */
vlc_module_begin()
    vlc_set_description("macOS Video Output Window")
    vlc_set_capability("vout window", 1)
    vlc_set_callback(Open)
    vlc_add_shortcut("macos-window")

    vlc_add_submodule()
    vlc_set_shortname(N_("NSObject Drawable"))
    vlc_set_subcategory(SUBCAT_VIDEO_VOUT)
    vlc_set_capability("vout window", 10)
    vlc_set_callback(EmbedOpen)
    vlc_add_shortcut("macos-embed")
vlc_module_end()
