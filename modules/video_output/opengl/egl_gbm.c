...
vlc_module_begin ()
    vlc_set_shortname (N_("EGL GBM Wayland"))
    vlc_set_description (N_("EGL GBM wayland extension for OpenGL"))
    vlc_set_subcategory (SUBCAT_VIDEO_VOUT)
    vlc_set_callback_opengl(OpenGL, 40) //lower priority than egl+wayland
    vlc_add_shortcut ("egl")

    vlc_add_submodule ()
    vlc_set_callback_opengl_es2(OpenGLES2, 40) //lower priority than egl+wayland
    vlc_add_shortcut ("egl")
