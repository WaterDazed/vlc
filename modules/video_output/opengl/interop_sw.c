...
vlc_module_begin ()
    vlc_set_description("Software OpenGL interop")
    vlc_set_capability("opengl sw interop", 1)
    vlc_set_callback(OpenInteropSW)
    vlc_set_subcategory(SUBCAT_VIDEO_VOUT)
    vlc_add_shortcut("sw")

    vlc_add_submodule()
    vlc_set_callback(OpenInteropDirectRendering)
    vlc_set_capability("opengl sw interop", 2)
    vlc_add_shortcut("pbo")
vlc_module_end ()
