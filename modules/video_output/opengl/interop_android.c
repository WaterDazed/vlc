...
ahb_cache_get(struct ahb_cache *pool, uint64_t ahb_id, bool *is_new)
{
    ...
    *is_new = true;
    ...
    /* Caller must clean up old api_data if present */
    return free_slot;
}
                 ...
                 picture_t *pic, const size_t plane_offset[])
{
    ...
    assert(pic->context);
    assert(textures[0] != 0);
    assert(priv->current_picture == NULL);
...
#undef LOAD_SYMBOL
    ...
    priv->use_aimage = ret == VLC_SUCCESS;

    /* Check if we have either AImageReader or SurfaceTexture available */
    bool has_stex = avctx->texture != NULL || avctx->get_texture != NULL;

    if (!priv->use_aimage && !has_stex)
    {
        msg_Err(&interop->obj, "Neither AImage nor SurfaceTexture available");
        free(priv);
        return VLC_EGENERIC;
    }
    ...
    {
        static const struct vlc_gl_interop_ops stex_ops = {
            .allocate_textures = tc_anop_allocate_textures,
            .update_textures = tc_anop_update,
            .get_transform_matrix = tc_get_transform_matrix,
            .close = Close,
        };
        interop->ops = &stex_ops;
    }
