...
vlc_module_begin()
    vlc_set_shortname(N_("File keystore (plaintext)"))
    vlc_set_description(N_("Secrets are stored on a file without any encryption"))
    vlc_set_subcategory(SUBCAT_ADVANCED_MISC)
    vlc_set_callbacks(Open, Close)
    vlc_add_savefile("keystore-file", NULL, NULL, NULL)
        vlc_change_private()
    vlc_set_capability("keystore", 0)
    vlc_add_shortcut("file_plaintext")
#ifdef CRYPTFILE
    vlc_add_submodule()
        vlc_set_shortname(N_("Crypt keystore"))
        vlc_set_description(N_("Secrets are stored encrypted on a file"))
        vlc_set_subcategory(SUBCAT_ADVANCED_MISC)
        vlc_set_callbacks(OpenCrypt, CloseCrypt)
        vlc_set_capability("keystore", 1)
        vlc_add_shortcut("file_crypt")
#endif
vlc_module_end ()
