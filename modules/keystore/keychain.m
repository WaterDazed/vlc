...
vlc_module_begin()
    vlc_set_shortname(N_("Keychain keystore"))
    vlc_set_description(N_("Keystore for iOS, macOS and tvOS"))
    vlc_set_subcategory(SUBCAT_ADVANCED_MISC)
    vlc_add_integer("keychain-synchronize", 1, SYNC_ITEMS_TEXT, SYNC_ITEMS_LONGTEXT)
    vlc_change_integer_list(sync_list, sync_list_text)
    vlc_add_integer("keychain-accessibility-type", 0, ACCESSIBILITY_TYPE_TEXT, ACCESSIBILITY_TYPE_LONGTEXT)
    vlc_change_integer_list(accessibility_list, accessibility_list_text)
    vlc_add_string("keychain-access-group", NULL, ACCESS_GROUP_TEXT, ACCESS_GROUP_LONGTEXT)
