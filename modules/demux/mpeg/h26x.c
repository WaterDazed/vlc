...
vlc_module_begin ()
    vlc_set_shortname( "H264")
    vlc_set_subcategory( SUBCAT_INPUT_DEMUX )
    vlc_set_description( N_("H264 video demuxer" ) )
    vlc_set_capability( "demux", 8 )
    vlc_set_section( N_("H264 video demuxer" ), NULL )
    vlc_add_float( "h264-fps", 0.0, FPS_TEXT, FPS_LONGTEXT )
    vlc_set_callbacks( OpenH264, Close )
    vlc_add_shortcut( "h264" )
    vlc_add_file_extension("h264")

    vlc_add_submodule()
        vlc_set_shortname( "HEVC")
        vlc_set_subcategory( SUBCAT_INPUT_DEMUX )
        vlc_set_description( N_("HEVC/H.265 video demuxer" ) )
        vlc_set_capability( "demux", 8 )
        vlc_set_section( N_("HEVC/H.265 video demuxer" ), NULL )
        vlc_add_float( "hevc-fps", 0.0, FPS_TEXT, FPS_LONGTEXT )
        vlc_set_callbacks( OpenHEVC, Close )
        vlc_add_shortcut( "hevc", "h265" )
