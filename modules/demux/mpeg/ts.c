...
vlc_module_begin ()
    vlc_set_description( N_("MPEG Transport Stream demuxer") )
    vlc_set_shortname ( "MPEG-TS" )
    vlc_set_subcategory( SUBCAT_INPUT_DEMUX )

    vlc_add_string( "ts-standard", "auto", STANDARD_TEXT, STANDARD_LONGTEXT )
        vlc_change_string_list( ts_standards_list, ts_standards_list_text )

    vlc_add_string( "ts-extra-pmt", NULL, PMT_TEXT, PMT_LONGTEXT )
    vlc_add_bool( "ts-trust-pcr", true, PCR_TEXT, PCR_LONGTEXT )
        vlc_change_safe()
    vlc_add_bool( "ts-es-id-pid", true, PID_TEXT, PID_LONGTEXT )
        vlc_change_safe()
    vlc_add_string( "ts-csa-ck", NULL, CSA_TEXT, CSA_LONGTEXT )
        vlc_change_safe()
    vlc_add_string( "ts-csa2-ck", NULL, CSA2_TEXT, CSA2_LONGTEXT )
        vlc_change_safe()
    vlc_add_integer( "ts-csa-pkt", 188, CPKT_TEXT, CPKT_LONGTEXT )
        vlc_change_safe()

    vlc_add_bool( "ts-split-es", true, SPLIT_ES_TEXT, SPLIT_ES_LONGTEXT )
    vlc_add_bool( "ts-seek-percent", false, SEEK_PERCENT_TEXT, SEEK_PERCENT_LONGTEXT )
    vlc_add_bool( "ts-cc-check", true, CC_CHECK_TEXT, CC_CHECK_LONGTEXT )
    vlc_add_bool( "ts-pmtfix-waitdata", true, TS_SKIP_GHOST_PROGRAM_TEXT, NULL )
    vlc_add_bool( "ts-patfix", true, TS_PATFIX_TEXT, NULL )
    vlc_add_bool( "ts-pcr-offsetfix", true, TS_OFFSETFIX_TEXT, NULL )
    vlc_add_integer_with_range( "ts-generated-pcr-offset", 120, 0, 500,
