...
vlc_module_begin ()
    ...
    vlc_set_description( N_("MP4 stream demuxer") )
    vlc_set_shortname( N_("MP4") )
    vlc_set_capability( "demux", 240 )
    vlc_set_callbacks( Open, Close )
    vlc_add_file_extension("m4a")
    vlc_add_file_extension("m4v")
    vlc_add_file_extension("moov")
    vlc_add_file_extension("mov")
    vlc_add_file_extension("mp4")

    vlc_set_section("Hacks", NULL)
    vlc_add_bool( CFG_PREFIX"m4a-audioonly", false, MP4_M4A_TEXT, MP4_M4A_LONGTEXT )
    vlc_add_bool( CFG_PREFIX"editlist", true, MP4_ELST_TEXT, MP4_ELST_TEXT )

    vlc_add_submodule()
        vlc_set_subcategory( SUBCAT_INPUT_DEMUX )
        vlc_set_description( N_("HEIF demuxer") )
        vlc_set_shortname( "heif" )
        vlc_set_capability( "demux", 239 )
        vlc_set_callbacks( OpenHEIF, CloseHEIF )
        vlc_set_section( N_("HEIF demuxer"), NULL )
        vlc_add_float( "heif-image-duration", HEIF_DEFAULT_DURATION,
