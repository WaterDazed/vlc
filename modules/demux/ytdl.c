...
vlc_module_begin()
    vlc_set_shortname("YT-DL")
    vlc_set_description("YT-DL extractor")
    vlc_set_subcategory(SUBCAT_INPUT_STREAM_FILTER)
    vlc_set_capability("demux", 7)
    vlc_set_callbacks(OpenFilter, Close)
    vlc_add_bool("ytdl", true, N_("Enable YT-DL"), NULL)
        vlc_change_safe()

    vlc_add_submodule()
    vlc_set_capability("access", 0)
    vlc_add_shortcut("ytdl")
    vlc_set_callbacks(OpenCommon, Close)
vlc_module_end()
