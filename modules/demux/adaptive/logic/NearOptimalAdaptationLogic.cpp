...
/*
 * Multi stream version of BOLA: Near-Optimal Bitrate Adaptation for Online
 * Videos http://arxiv.org/abs/1601.06748
 */
...
NearOptimalContext::NearOptimalContext()
	: buffering_min(minimumBufferS), buffering_level(0),
	  buffering_target(bufferTargetS), last_download_rate(0) {}

NearOptimalAdaptationLogic::NearOptimalAdaptationLogic(vlc_object_t *obj)
	: AbstractAdaptationLogic(obj), currentBps(0), usedBps(0), p_obj(obj) {
	vlc_mutex_init(&lock);
}
...
BaseRepresentation *NearOptimalAdaptationLogic::getNextQualityIndex(
	BaseAdaptationSet *adaptSet, RepresentationSelector &selector, float gammaP,
	vlc_tick_t VD, vlc_tick_t Q) {
...
BaseRepresentation *
NearOptimalAdaptationLogic::getNextRepresentation(BaseAdaptationSet *adaptSet,
	...
	std::map<ID, NearOptimalContext>::iterator it =
		streams.find(adaptSet->getID());
	if (it == streams.end()) {
	...
	const float gammaP =
		1.0 +
		(umax - umin) /
			((float)ctxcopy.buffering_target / ctxcopy.buffering_min - 1.0);
	const float Vd =
		((float)ctxcopy.buffering_min / CLOCK_FREQ - 1.0) / (umin + gammaP);

	BaseRepresentation *m;
	if (prevRep == nullptr) { /* Starting */
		m = selector.select(adaptSet, bps);
		if (m == lowest) {
			/* Handle HLS specific cases where the lowest is audio only. Try to
			 * pick first A+V */
			BaseRepresentation *n = selector.higher(adaptSet, m);
			if (m != n && m->getCodecs().size() == 1 &&
				n->getCodecs().size() > 1)
				m = n;
		}
	} else {
		/* noted m* */
		m = getNextQualityIndex(
			adaptSet, selector,
			gammaP - umin /* umin == Sm, utility = std::log(S/Sm) */, Vd,
			(float)ctxcopy.buffering_level / CLOCK_FREQ);
	...
	BwDebug(msg_Info(
				p_obj, "buffering level %.2f%% rep %" PRId64 " kBps %u kBps",
				(float)100 * ctxcopy.buffering_level / ctxcopy.buffering_target,
				m->getBandwidth() / 8000, bps / 8000););
...
float NearOptimalAdaptationLogic::getUtility(const BaseRepresentation *rep) {
	...
	std::map<uint64_t, float>::iterator it =
		utilities.find(rep->getBandwidth());
	if (it == utilities.end()) {
...
unsigned NearOptimalAdaptationLogic::getAvailableBw(
	unsigned i_bw, const BaseRepresentation *curRep) const {
...
unsigned NearOptimalAdaptationLogic::getMaxCurrentBw() const {
	...
	for (std::map<ID, NearOptimalContext>::const_iterator it = streams.begin();
		 it != streams.end(); ++it)
		i_max_bitrate =
			std::max(i_max_bitrate, ((*it).second).last_download_rate);
...
void NearOptimalAdaptationLogic::updateDownloadRate(const ID &id, size_t dlsize,
	...
	if (it != streams.end()) {
		...
		ctx.last_download_rate =
			ctx.average.push(CLOCK_FREQ * dlsize * 8 / time);
	}
...
void NearOptimalAdaptationLogic::trackerEvent(const TrackerEvent &ev) {
	...
	case TrackerEvent::Type::RepresentationSwitch: {
		...
		BwDebug(msg_Info(p_obj, "New total bandwidth usage %u kBps",
						 (usedBps / 8000)));
		vlc_mutex_unlock(&lock);
	} break;
