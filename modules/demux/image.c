...
vlc_module_begin()
    vlc_set_description(N_("Image demuxer"))
    vlc_set_shortname(N_("Image"))
    vlc_set_subcategory(SUBCAT_INPUT_DEMUX)
    vlc_add_integer("image-id", -1, ID_TEXT, ID_LONGTEXT)
        vlc_change_safe()
    vlc_add_integer("image-group", 0, GROUP_TEXT, GROUP_LONGTEXT)
        vlc_change_safe()
    vlc_add_bool("image-decode", true, DECODE_TEXT, DECODE_LONGTEXT)
        vlc_change_safe()
    vlc_add_string("image-chroma", NULL, CHROMA_TEXT, CHROMA_LONGTEXT)
        vlc_change_safe()
    vlc_add_float("image-duration", 10, DURATION_TEXT, DURATION_LONGTEXT)
        vlc_change_safe()
    vlc_add_string("image-fps", "10/1", FPS_TEXT, FPS_LONGTEXT)
        vlc_change_safe()
    vlc_add_bool("image-realtime", false, RT_TEXT, RT_LONGTEXT)
        vlc_change_safe()
