...
vlc_module_begin ()
    vlc_set_description( N_("TS muxer (libdvbpsi)") )
    vlc_set_shortname( "MPEG-TS")
    vlc_set_subcategory( SUBCAT_SOUT_MUX )
    vlc_set_capability( "sout mux", 120 )
    vlc_add_shortcut( "ts" )

    vlc_add_string( SOUT_CFG_PREFIX "standard", "dvb", STANDARD_TEXT, NULL )
        vlc_change_string_list( ts_standards_list, ts_standards_list_text )
    vlc_add_integer(SOUT_CFG_PREFIX "pid-video", 100, VPID_TEXT, VPID_LONGTEXT)
        vlc_change_integer_range( 32, 8190 )
    vlc_add_integer(SOUT_CFG_PREFIX "pid-audio", 200, APID_TEXT, APID_LONGTEXT)
        vlc_change_integer_range( 32, 8190 )
    vlc_add_integer(SOUT_CFG_PREFIX "pid-spu",   300, SPUPID_TEXT, SPUPID_LONGTEXT)
        vlc_change_integer_range( 32, 8190 )
    vlc_add_integer(SOUT_CFG_PREFIX "pid-pmt", 32, PMTPID_TEXT, PMTPID_LONGTEXT)
        vlc_change_integer_range( 32, 8190 )
    vlc_add_integer(SOUT_CFG_PREFIX "tsid",  0, TSID_TEXT, TSID_LONGTEXT)
    vlc_add_integer(SOUT_CFG_PREFIX "netid", 0, NETID_TEXT, NETID_LONGTEXT)
    vlc_add_string(SOUT_CFG_PREFIX "program-pmt", NULL, PMTPROG_TEXT, PMTPROG_LONGTEXT)
    vlc_add_bool(SOUT_CFG_PREFIX "es-id-pid", false, PID_TEXT, PID_LONGTEXT)
    vlc_add_string(SOUT_CFG_PREFIX "muxpmt",  NULL, MUXPMT_TEXT, MUXPMT_LONGTEXT)
    vlc_add_string(SOUT_CFG_PREFIX "sdtdesc", NULL, SDTDESC_TEXT, SDTDESC_LONGTEXT)
    vlc_add_bool(SOUT_CFG_PREFIX "alignment", true, ALIGNMENT_TEXT, ALIGNMENT_LONGTEXT)

    vlc_add_integer(SOUT_CFG_PREFIX "shaping", 200, SHAPING_TEXT, SHAPING_LONGTEXT)
    vlc_add_bool(SOUT_CFG_PREFIX "use-key-frames", false, KEYF_TEXT, KEYF_LONGTEXT)

    vlc_add_integer( SOUT_CFG_PREFIX "pcr", 70, PCR_TEXT, PCR_LONGTEXT)
    vlc_add_integer( SOUT_CFG_PREFIX "dts-delay", 400, DTS_TEXT, DTS_LONGTEXT)

    vlc_add_obsolete_integer( "sout-ts-bmin" ) /* since 4.0.0 */
    vlc_add_obsolete_integer( "sout-ts-bmax" ) /* since 4.0.0 */

    vlc_add_bool( SOUT_CFG_PREFIX "crypt-audio", true, ACRYPT_TEXT, ACRYPT_LONGTEXT)
    vlc_add_bool( SOUT_CFG_PREFIX "crypt-video", true, VCRYPT_TEXT, VCRYPT_LONGTEXT)
    vlc_add_string( SOUT_CFG_PREFIX "csa-ck",  NULL, CK_TEXT,   CK_LONGTEXT)
    vlc_add_string( SOUT_CFG_PREFIX "csa2-ck", NULL, CK2_TEXT,  CK2_LONGTEXT)
    vlc_add_string( SOUT_CFG_PREFIX "csa-use", "1",  CU_TEXT,   CU_LONGTEXT)
    vlc_add_integer(SOUT_CFG_PREFIX "csa-pkt", 188,  CPKT_TEXT, CPKT_LONGTEXT)
