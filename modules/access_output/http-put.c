...
vlc_module_begin()
    vlc_set_description(N_("HTTP PUT stream output"))
    vlc_set_shortname(N_("HTTP PUT"))
    vlc_set_capability("sout access", 0)
    vlc_set_subcategory(SUBCAT_SOUT_ACO)
    vlc_add_shortcut("http-put")
    vlc_add_string(SOUT_CFG_PREFIX"user", NULL, N_("Username"), NULL)
    vlc_add_password(SOUT_CFG_PREFIX"pwd", NULL, N_("Password"), NULL)
