                                          ...
                                          h266_profile_tier_level_t *p)
{
    ...
    if(profileTierPresentFlag)
    {
        ...
        for(uint8_t i=0; i<p->ptl_num_sub_profiles; i++)
...
struct h266_sequence_parameter_set_t
{
    ...
    nal_u1_t long_term_ref_pics_flag;
                                                ...
                                                h266_video_parameter_set_t *p_vps)
{
        ...
        if(!p_vps->vps_each_layer_is_an_ols_flag)
        {
            if(!p_vps->vps_all_independent_layers_flag)
            {
                ...
                if(vps_ols_mode_idc == 2)
                {
                 ...
                 for(nal_u8_t i=1; i<= p_vps->vps_num_output_layer_sets_minus2 +1; i++)
    ...
    for(nal_u8_t i=0; i<= p_vps->vps_num_ptls_minus1; i++)
    {
        if(i > 0)
    ...
    for(nal_u8_t i=0; i< TotalNumOlss; i++)
    {
    ...
    if(!p_vps->vps_each_layer_is_an_ols_flag)
    {
        ...
        nal_u8_t VpsNumDpbParams = p_vps->vps_each_layer_is_an_ols_flag
                                 ? 0 : p_vps->vps_num_dpb_params_minus1 + 1;
        for(nal_u8_t i=0; i< VpsNumDpbParams; i++)
        {
                                   ...
                                   h266_sequence_parameter_set_t *p_sps)
{
    ...
    if(!p_sps->long_term_ref_pics_flag &&
        ...
        if(bs_read(p_bs, 1))
        {
            if(bs_read(p_bs, 1)) // sps_subpic_id_mapping_present_flag
            {
                for(nal_ue_t i=0; i<= sps_num_subpics_minus1; i++)
                    bs_skip(p_bs, sps_subpic_id_len_minus1);
            }
        }
    }
                                           ...
                                           h266_picture_header_t *ph)
{
    ...
    if(bs_read1(p_bs)) // ph_inter_slice_allowed_flag
        bs_read1(p_bs);
...
bool h266_get_slice_type(const h266_picture_header_t *ph, enum h266_slice_type_e *type)
{
    ...
    else if(ph->ph_non_ref_pic_flag)
        *type = H266_SLICE_TYPE_B;
    else
