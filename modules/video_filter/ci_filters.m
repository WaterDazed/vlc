...
vlc_module_begin()
    ...
    vlc_set_description(N_("Mac OS X hardware video filters"))
    ...
    vlc_add_submodule()
    vlc_set_callback_video_filter(OpenInvert)
    vlc_add_shortcut("invert")

    vlc_add_submodule()
    vlc_set_callback_video_filter(OpenPosterize)
    vlc_add_shortcut("posterize")

    vlc_add_submodule()
    vlc_set_callback_video_filter(OpenSepia)
    vlc_add_shortcut("sepia")

    vlc_add_submodule()
    vlc_set_callback_video_filter(OpenSharpen)
    vlc_add_shortcut("sharpen")

    vlc_add_submodule()
    vlc_set_callback_video_filter(OpenPsychedelic)
    vlc_add_shortcut("psychedelic")

    vlc_add_submodule()
    vlc_set_callback_video_filter(OpenCustom)
    vlc_add_shortcut("ci")
    vlc_add_string("ci-filter", "CIComicEffect", CI_CUSTOM_FILTER_TEXT, CI_CUSTOM_FILTER_LONGTEXT)
vlc_module_end()
