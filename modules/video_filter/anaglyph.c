...
vlc_module_begin()
    vlc_set_description(N_("Convert 3D picture to anaglyph image video filter"))
    vlc_set_shortname(N_("Anaglyph"))
    vlc_set_subcategory(SUBCAT_VIDEO_VFILTER)
    vlc_add_string(FILTER_PREFIX "scheme", "red-cyan", SCHEME_TEXT, SCHEME_LONGTEXT)
        vlc_change_string_list(ppsz_scheme_values, ppsz_scheme_descriptions)
