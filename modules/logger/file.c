...
vlc_module_begin()
    vlc_set_shortname(N_("Logger"))
    vlc_set_description(N_("File logger"))
    vlc_set_subcategory(SUBCAT_ADVANCED_MISC)
    vlc_set_capability("logger", 15)
    vlc_set_callback(Open)

    vlc_add_bool("file-logging", false, FILE_LOG_TEXT, FILE_LOG_LONGTEXT)
    vlc_add_savefile("logfile", NULL, LOGFILE_NAME_TEXT, LOGFILE_NAME_LONGTEXT)
    vlc_add_string("logmode", "text", LOGMODE_TEXT, LOGMODE_LONGTEXT)
        vlc_change_string_list(mode_list, mode_list_text)
    vlc_add_integer("log-verbose", -1, LOGVERBOSE_TEXT, LOGVERBOSE_LONGTEXT)
        vlc_change_integer_list(verbosity_values, verbosity_text)
vlc_module_end ()
