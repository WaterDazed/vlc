...
vlc_module_begin ()
    vlc_set_description( N_("Clone video filter") )
    vlc_set_capability( "video splitter", 0 )
    vlc_set_shortname( N_("Clone" ))
    vlc_set_help(CLONE_HELP)
    vlc_set_subcategory( SUBCAT_VIDEO_SPLITTER )

    vlc_add_integer( CFG_PREFIX "count", 2, COUNT_TEXT, COUNT_LONGTEXT )
    vlc_add_module_list(CFG_PREFIX "vout-list", "vout display", NULL,
                    VOUTLIST_TEXT, VOUTLIST_LONGTEXT)

    vlc_add_shortcut( "clone" )
