...
protected:
    ...
    void mouseMoveEvent(QMouseEvent *event) override {
        if (m_player) {
            ...
            m_player->set_mouse_move(pos.x(), pos.y());
        }
        event->accept();
    }
    ...
    void mousePressEvent(QMouseEvent *event) override {
        if (m_player) {
            m_player->set_mouse_press(getMouseButton(event));
        }
        event->accept();
    }
    ...
    void mouseReleaseEvent(QMouseEvent *event) override {
        if (m_player) {
            m_player->set_mouse_release(getMouseButton(event));
        }
        event->accept();
    }
...
private:
    ...
    libvlc_video_output_mouse_button_t getMouseButton(QMouseEvent *event) {
        switch (event->button()) {
            case Qt::LeftButton: return libvlc_video_output_mouse_button_left;
            case Qt::RightButton: return libvlc_video_output_mouse_button_right;
            case Qt::MiddleButton: return libvlc_video_output_mouse_button_middle;
            default: assert(0);
        }
    }
    ...
    void initializeVLC() {
        ...
        m_player->set_render_window(display, surface);
        m_player->play();
        m_initialized = true;
    }
...
int main(int argc, char *argv[])
{
    ...
    mainWidget.resize(VlcPlayer::width, VlcPlayer::height);
