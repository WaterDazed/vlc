...
vlc_preparser_ended(vlc_preparser_req *req, int status, void *user_data)
{
    ...
    atomic_store(&task->media->parsed_status, new_status);

    task->cbs.parser->on_parsed(
        task->cbs_opaque, task, new_status);
                    ...
                    void *cbs_opaque)
{
    ...
    assert(req->version <= LIBVLC_PARSER_REQUEST_VER_LATEST);
    assert(cbs->version <= LIBVLC_PARSER_CBS_VER_LATEST);
    ...
    while (
        !atomic_compare_exchange_weak(&media->parsed_status,
                                      &expected,
                                 ...
                                 void *cbs_opaque)
{
    ...
    /* No different versions to handle for now */
    assert(req->version <= LIBVLC_THUMBNAILER_REQUEST_VER_LATEST);
    assert(cbs->version <= LIBVLC_THUMBNAILER_CBS_VER_LATEST);
    ...
    thumb_arg.seek.type = seek_type;
    thumb_arg.seek.speed = req->seek.speed;
    if (seek_type == libvlc_thumbnailer_seek_time)
        thumb_arg.seek.time = vlc_tick_from_libvlc_time(req->seek.time);
    else if (seek_type == libvlc_thumbnailer_seek_pos)
        thumb_arg.seek.pos = req->seek.pos;
