                                                     ...
                                                     libvlc_downloader_status_t new_status)
{
    ...
    if (task->cbs->on_state_update && status_changed)
...
downloaderThread(void *data)
{
    ...
    while (true)
    {
        ...
        if (consumed_offset == buf_len)
        {
            ...
            vlc_stream_GetSize(task->s, &stream_size); /* update size in case it changed */
...
libvlc_downloader_new(libvlc_instance_t *inst, const struct libvlc_downloader_cfg *cfg)
{
    ...
    assert(cfg->version <= LIBVLC_DOWNLOADER_CFG_VER_LATEST);
...
static void parse_ended(void *opaque, libvlc_parser_task *parser_task, libvlc_media_parsed_status_t status)
{
    (void) parser_task;
    ...
    if (status == libvlc_media_parsed_status_cancelled)
    {
...
struct libvlc_parser_cbs parser_cbs = {
                        ...
                        const struct libvlc_downloader_cbs *cbs, void *cbs_opaque)
{
    ...
    assert(cbs != NULL && cbs->on_buffer != NULL);

    /* No different versions to handle for now */
    assert(req->version <= LIBVLC_DOWNLOADER_REQUEST_VER_LATEST);
    assert(cbs->version <= LIBVLC_DOWNLOADER_CBS_VER_LATEST);
